...
export class PressureEngine {
  ...
  applyPressure(event: PressureEvent, evidence: Evidence[] = []): PressureResult {
    ...
    const maxDepth     = refinedHops.reduce((m, h) => {
      // Reconstituer la profondeur en cherchant la chaîne de hops
      return m  // simplified — depth tracking is in propagateWave but not exported in hop
    }, 0)
  ...
  simulatePressure(event: PressureEvent, evidence: Evidence[] = []): PressureSimulation {
    ...
    // Appliquer sans log (flag via event cloné)
    const result = this._applyWithoutLog(event, evidence)
